...
description: "Stable contracts for the Corpo protocol. Defines file format, corpus structure, cache and sync model, and auth extension points. All implementations — CLI, viewer, adapters — must conform to these primitives."
---

_See also: [Discovery](discovery.md) · [User Flows](user-flows.md)_

Stable contracts for the Corpo protocol. All implementations — CLI, viewer,
...
| Term | Definition |
|---|---|
| **File** | A Markdown file with YAML frontmatter, named `{file-id}.md`. Files are either **local** (owned by the current corpus) or **remote** (fetched from an external corpus). |
| **Thread** | An inline comment thread attached to a position in a file's body. |
| **Corpus** | A collection of files. Local files live in `corpus/`; remote files are declared by ID and fetched on demand. Governed by `corpo.yaml` at the corpus level or `~/.corpo/config.yaml` at the user level. May declare an `entrypoint` file ID for agent orientation. |
...
description: "A concise plain-text summary of this file. Capped at 1024
  characters. Written for agents and humans skimming the corpus."
...
the same `import` URL already exists in any configured corpus.
...
A corpus contains two kinds of files: **local files** that live in the
`corpus/` directory and are owned by this corpus, and **remote files** declared
in `corpo.yaml` that are fetched from external corpora on demand. Any corpus
can have local files, remote files, or both.
...
`corpo.yaml` governs the corpus. `corpo init` creates it alongside the local
files directory.
...
```yaml
# corpo.yaml
local_files_root: corpus
entrypoint: 550e8400e29b41d4a716446655440000

local_files:
  550e8400e29b41d4a716446655440000: .
  3f7a9b2c1e4d5f6a7b8c9d0e1f2a3b4c: strategy
remote_files:
...
| Key | Description |
|---|---|
| `local_files_root` | Required. Local files directory. `corpo init` always sets this to `corpus`. |
| `entrypoint` | Optional. A file ID — local or remote — that agents should read first to orient on this corpus. |
| `local_files` | Maps local file IDs to their directory path relative to `local_files_root`. Use `.` for files at the root of `local_files_root`. |
...
Within `local_files_root`, structure is optional and freeform. Flat is fine for small
corpora. Teams may use subdirectories to mirror their org hierarchy.

For git remote consumers, `corpo.yaml` is the entry point: fetch it once,
read the `local_files` map to resolve the directory for any local ID, then
fetch `{local_files_root}/{dir}/{file-id}.md` directly.
...
The CLI stitches together local and remote corpora through a user-level cache.
All cached content lives at the user level — there is no per-corpus data store.
...
`~/.corpo/config.yaml` defines the user-level defaults applied in all
...
applied as a fallback when no `corpo.yaml` is present or when an ID is not
found in `remote_files`.

### 3.3 Cache Layout

```
~/.corpo/
  config.yaml        # user config: remotes
...
partial state. `path` is cached for display purposes — it shows users where
the file lives in the remote as they would see it in a repo browser or viewer.
The CLI does not use `path` for fetching; git remotes resolve location via
`corpo.yaml`'s `local_files` map and registry remotes use their API.
...
- `corpo sync` for any file in `corpo.yaml` `remote_files` not yet in cache
...
to `corpo.yaml` `remote_files` but not immediately fetched. Fetch happens on
next explicit read or sync.

### 3.6 Staleness and Sync

**Git remotes** — `corpo sync` uses the corpus index as its entry point:
...
3. If changed → fetch `corpo.yaml` from the remote
4. For each ID present in `corpo.yaml` `remote_files`, compare the file's
   blob SHA against the cached `blob_sha`
5. Download only files where SHA differs
...
When the CLI detects `corpo.yaml`, it background-syncs all files in
...
- **Viewer auth** — controls access to the rendered browser view
