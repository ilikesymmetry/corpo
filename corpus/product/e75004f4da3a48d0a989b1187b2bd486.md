...
| **Corpus** | A collection of files. Local files live in `corpus/`; remote files are declared by ID and fetched on demand. Governed by a config that lives at the corpus level (`corpo.yaml`) or user level (`~/.corpo/config.yaml`). |
...
fetch `{root}/{dir}/{file-id}.md` directly.
