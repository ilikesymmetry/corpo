...
export function createServer(root: string): Hono {
  ...
  // Hashed static assets (JS, CSS) from gui/dist/assets/
  app.use('/assets/*', serveStatic({ root: join(import.meta.dir, '../../gui/dist') }))

  // SPA shell — all other routes return index.html (client-side routing handles the rest)
  app.get('*', async (c) => {
    const index = await readFile(join(import.meta.dir, '../../gui/dist/index.html'), 'utf8')
    return c.html(index)
  })
