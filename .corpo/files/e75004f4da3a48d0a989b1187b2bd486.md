...
| **Corpus** | A collection of files. Local files live in `.corpo/files/`; remote files are declared by ID and fetched on demand. Governed by `.corpo/config.json` at the corpus level or `~/.corpo/config.json` at the user level. |
...
A corpus contains two kinds of files: **local files** that live in
`.corpo/files/` and are owned by this corpus, and **remote files** declared
in `.corpo/config.json` that are fetched from external corpora on demand. Any
corpus can have local files, remote files, or both.
...
`.corpo/config.json` governs the corpus. `corpo init` creates the `.corpo/`
...
directory and it is part of the corpus. Add it to `navigation` to surface it
in the sidebar.
...
implicit starting point for agents and humans orienting on the corpus.
